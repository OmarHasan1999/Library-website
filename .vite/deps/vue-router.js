import { $ as getCurrentInstance, Et as provide, F as computed, J as defineComponent, Sn as ref, St as onUnmounted, Tn as shallowRef, Xt as watch, Zt as watchEffect, at as inject, bn as reactive, dt as nextTick, ft as onActivated, gt as onDeactivated, jn as unref, nt as h } from "./vue.runtime.esm-bundler-C5CmcPWT.js";
...
function normalizeRecordProps(record) {
	...
	else for (const name in record.components) propsObject[name] = typeof props === "boolean" ? props : props[name];
	return propsObject;
}
/**
	...
	*/
	...
	function navigate(to, from) {
		...
		return runGuardQueue(guards).then(() => {
			...
			guards = [];
			for (const record of to.matched) if (record.beforeEnter && !from.matched.includes(record)) if (isArray(record.beforeEnter)) for (const beforeEnter of record.beforeEnter) guards.push(guardToPromiseFn(beforeEnter, to, from));
