...
var isGloballyWhitelisted = /* @__PURE__ */ makeMap("Infinity,undefined,NaN,isFinite,isNaN,parseFloat,parseInt,decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,Math,Number,Date,Array,Object,Boolean,String,RegExp,Map,Set,JSON,Intl,BigInt");
...
var replacer = (_key, val) => {
	...
	else if (isMap(val)) return { [`Map(${val.size})`]: [...val.entries()].reduce((entries, [key, val]) => {
		entries[`${key} =>`] = val;
		return entries;
	}, {}) };
...
function onScopeDispose(fn) {
	...
	else warn$1("onScopeDispose() is called when there is no active effect scope to be associated with.");
}
...
function trackEffects(dep, debuggerEventExtraInfo) {
	...
	if (shouldTrack) {
		...
		if (activeEffect.onTrack) activeEffect.onTrack(Object.assign({ effect: activeEffect }, debuggerEventExtraInfo));
	}
}
...
var shallowReadonlyHandlers = /* @__PURE__ */ extend({}, readonlyHandlers, { get: shallowReadonlyGet });
	...
	const props = instance.vnode.props || EMPTY_OBJ;
	{
		...
		if (emitsOptions) if (!(event in emitsOptions) && true) {
			...
			if (isFunction(validator)) {
				if (!validator(...rawArgs)) warn(`Invalid event arguments: event validation failed for event "${event}".`);
			}
		}
	}
	let args = rawArgs;
	...
	const suspense = {
		...
		resolve(resume = false) {
			...
			if (suspense.isHydrating) suspense.isHydrating = false;
				...
				if (delayEnter) activeBranch.transition.afterLeave = () => {
					if (pendingId === suspense.pendingId) move(pendingBranch, container, anchor, 0);
				};
				let { anchor } = suspense;
				if (activeBranch) {
		...
		registerDep(instance, setupRenderEffect) {
			...
			instance.asyncDep.catch((err) => {
				...
				setupRenderEffect(instance, vnode, parentNode(hydratedEl || instance.subTree.el), hydratedEl ? null : next(instance.subTree), suspense, isSVG, optimized);
...
function injectHook(type, hook, target = currentInstance, prepend = false) {
	if (target) {
		...
		return wrappedHook;
	} else warn(`${toHandlerKey(ErrorTypeStrings[type].replace(/ hook$/, ""))} is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup(). If you are using async setup(), make sure to register lifecycle hooks before the first await statement.`);
}
...
var publicPropertiesMap = /* @__PURE__ */ extend(Object.create(null), {
	$: (i) => i,
	$el: (i) => i.vnode.el,
	$data: (i) => i.data,
...
var PublicInstanceProxyHandlers = {
	get({ _: instance }, key) {
		...
		if (publicGetter) {
			...
			if (data !== EMPTY_OBJ && isReservedPrefix(key[0]) && hasOwn(data, key)) warn(`Property ${JSON.stringify(key)} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`);
...
function applyOptions(instance) {
	...
	if (dataOptions) {
		...
		if (!isObject(data)) warn(`data() should return an object.`);
		else {
...
function resolveInjections(injectOptions, ctx, checkDuplicateProperties = NOOP, unwrapRef = false) {
	...
	for (const key in injectOptions) {
		...
		if (isRef(injected)) if (unwrapRef) Object.defineProperty(ctx, key, {
			enumerable: true,
			configurable: true,
			get: () => injected.value,
...
var updateSlots = (instance, children, optimized) => {
	...
	if (vnode.shapeFlag & 32) {
		const type = children._;
		if (type) if (isHmrUpdating) extend(slots, children);
	...
	const hydrateNode = (node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized = false) => {
		...
		switch (type) {
			...
			case Fragment:
				...
				break;
			default: if (shapeFlag & 1) if (domType !== 1 || vnode.type.toLowerCase() !== node.tagName.toLowerCase()) nextNode = onMismatch();
	...
	const locateClosingAsyncAnchor = (node) => {
		...
		while (node) {
			...
			if (node && isComment(node)) {
				if (node.data === "[") match++;
				if (node.data === "]") if (match === 0) return nextSibling(node);
				else match--;
			}
		}
		return node;
...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const patch = (n1, n2, container, anchor = null, parentComponent = null, parentSuspense = null, isSVG = false, slotScopeIds = null, optimized = isHmrUpdating ? false : !!n2.dynamicChildren) => {
		...
		switch (type) {
			...
			case Fragment:
				...
				break;
			default: if (shapeFlag & 1) processElement(n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized);
	...
	*/
	...
	const mountChildren = (children, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized, start = 0) => {
		for (let i = start; i < children.length; i++) patch(null, children[i] = optimized ? cloneIfMounted(children[i]) : normalizeVNode(children[i]), container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized);
	};
	...
	const patchBlockChildren = (oldChildren, newChildren, fallbackContainer, parentComponent, parentSuspense, isSVG, slotScopeIds) => {
		for (let i = 0; i < newChildren.length; i++) {
			...
			patch(oldVNode, newVNode, oldVNode.el && (oldVNode.type === Fragment || !isSameVNodeType(oldVNode, newVNode) || oldVNode.shapeFlag & 70) ? hostParentNode(oldVNode.el) : fallbackContainer, null, parentComponent, parentSuspense, isSVG, slotScopeIds, true);
		}
	};
	...
	const processComponent = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
		...
		if (n1 == null) if (n2.shapeFlag & 512) parentComponent.ctx.activate(n2, container, anchor, isSVG, optimized);
	...
	if (setup) {
		...
		if (isPromise(setupResult)) {
			...
			if (isSSR) return setupResult.then((resolvedResult) => {
				...
				if (!instance.suspense) warn(`Component <${(_a = Component.name) !== null && _a !== void 0 ? _a : "Anonymous"}>: setup function returned a promise, but no <Suspense> boundary was found in the parent component tree. A component with async setup() must be nested in a <Suspense> in order to be rendered.`);
			}
...
function finishComponentSetup(instance, isSSR, skipOptions) {
 ...
 /* istanbul ignore if */
	if (!compile$1 && Component.template) warn("Component provided template option but runtime compilation is not supported in this build of Vue. Configure your bundler to alias \"vue\" to \"vue/dist/vue.esm-bundler.js\".");
...
function mergeDefaults(raw, defaults) {
	...
	for (const key in defaults) {
		const opt = props[key];
		if (opt) if (isArray(opt) || isFunction(opt)) props[key] = {
			type: opt,
...
function patchDOMProp(el, key, value, prevChildren, parentComponent, parentSuspense, unmountChildren) {
	...
	if (key === "value" && el.tagName !== "PROGRESS" && !el.tagName.includes("-")) {
		...
		if (el.value !== newValue || el.tagName === "OPTION") el.value = newValue;
...
function setSelected(el, value) {
	...
	for (let i = 0, l = el.options.length; i < l; i++) {
		...
		if (isMultiple) if (isArray(value)) option.selected = looseIndexOf(value, optionValue) > -1;
...
//#endregion
export { getCurrentInstance as $, watchSyncEffect as $t, assertNumber as A, triggerRef as An, renderList as At, createHydrationRenderer as B, ssrContextKey as Bt, Comment as C, shallowReactive as Cn, onUpdated as Ct, Suspense as D, toRaw as Dn, pushScopeId as Dt, Static as E, stop as En, provide as Et, computed as F, normalizeProps as Fn, resolveFilter as Ft, createTextVNode as G, useSSRContext as Gt, createRenderer as H, toHandlers as Ht, createBaseVNode as I, normalizeStyle as In, resolveTransitionHooks as It, defineComponent as J, version as Jt, createVNode as K, useSlots as Kt, createBlock as L, toDisplayString as Ln, setBlockTracking as Lt, callWithErrorHandling as M, camelize as Mn, resolveComponent as Mt, cloneVNode as N, capitalize as Nn, resolveDirective as Nt, Teleport as O, toRef as On, queuePostFlushCb as Ot, compatUtils as P, normalizeClass as Pn, resolveDynamicComponent as Pt, devtools as Q, watchPostEffect as Qt, createCommentVNode as R, toHandlerKey as Rn, setDevtoolsHook as Rt, BaseTransition as S, ref as Sn, onUnmounted as St, KeepAlive as T, shallowRef as Tn, popScopeId as Tt, createSlots as U, transformVNodeArgs as Ut, createPropsRestProxy as V, ssrUtils as Vt, createStaticVNode as W, useAttrs as Wt, defineExpose as X, watch as Xt, defineEmits as Y, warn as Yt, defineProps as Z, watchEffect as Zt, vModelSelect as _, markRaw as _n, onErrorCaptured as _t, createApp as a, withScopeId as an, inject as at, withKeys as b, reactive as bn, onRenderTriggered as bt, defineSSRCustomElement as c, customRef as cn, isVNode as ct, render as d, getCurrentScope as dn, nextTick as dt, withAsyncContext as en, getTransitionRawChildren as et, useCssModule as f, isProxy as fn, onActivated as ft, vModelRadio as g, isShallow$1 as gn, onDeactivated as gt, vModelDynamic as h, isRef as hn, onBeforeUpdate as ht, VueElement as i, withMemo as in, initCustomFormatter as it, callWithAsyncErrorHandling as j, unref as jn, renderSlot as jt, Text as k, toRefs as kn, registerRuntimeCompiler as kt, hydrate as l, effect as ln, mergeDefaults as lt, vModelCheckbox as m, isReadonly as mn, onBeforeUnmount as mt, Transition as n, withDefaults as nn, h as nt, createSSRApp as o, EffectScope as on, isMemoSame as ot, useCssVars as p, isReactive as pn, onBeforeMount as pt, defineAsyncComponent as q, useTransitionState as qt, TransitionGroup as r, withDirectives as rn, handleError as rt, defineCustomElement as s, ReactiveEffect as sn, isRuntimeOnly as st, compile as t, withCtx as tn, guardReactiveProps as tt, initDirectivesForSSR as u, effectScope as un, mergeProps as ut, vModelText as v, onScopeDispose as vn, onMounted as vt, Fragment as w, shallowReadonly as wn, openBlock as wt, withModifiers as x, readonly as xn, onServerPrefetch as xt, vShow as y, proxyRefs as yn, onRenderTracked as yt, createElementBlock as z, setTransitionHooks as zt };

//# sourceMappingURL=vue.runtime.esm-bundler-C5CmcPWT.js.map